...
for ((i=$start;$i<=$end;i++)); do
	...
	### call the mapper
	 if [ "$i" = "$start" ] ; then
	    /nfs/users/nfs_m/mh12/git/python3/map_splitter.py -o " -i 1000 -r 0 -x -y 0.8 " -M 8 -m 1.2 --setup_mem 6.5 -c ICORN2.MAP.$$.$i -k 13 -s 3 smalt $reference.$i $directory $read1 $read2
	else
	    /nfs/users/nfs_m/mh12/git/python3/map_splitter.py -o " -i 1000 -r 0 -x -y 0.8 " -M 8 -m 1.2 --setup_mem 6.5 --depend ICORN.CORRECT.$$.$(($i-1)) -c ICORN2.MAP.$$.$i -k 13 -s 3 smalt $reference.$i $directory $read1 $read2
	fi
